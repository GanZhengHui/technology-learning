...
1. 存储数据大小方面：cookie数据大小不能超过4k，而localStorage和SessionStorage在5M甚至之上
2. 保留时间方面：cookie在cookie设置的过期时间之前一直有效，即窗口或者浏览器关闭
3. 数据与服务器之间的交互方式：cookie的数据会自动地传给服务器，而本地或会话存储不会，仅在本地存储
...
1. 浅拷贝：拷贝的是栈中的地址，二者指向同一内存，共享内存
   ...
   实现方式：Object.assign     const obj = Object.assign({},Obj)  注意，当拷贝的对象只有一层时是深拷贝
